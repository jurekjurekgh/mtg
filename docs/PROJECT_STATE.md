...
- **Faza:** Etapy 1–4 zamknięte na katalogu syntetycznym; M5 (pierwsza pionowa ścieżka UI)
  ...
  inspektor grobów, tokeny i załączniki, szkielet frameworku abilities, podgląd karty
  z ilustracją, autosave i wznawianie partii.
...
- framework abilities (activated/triggered/static), tokeny i załączniki —
  szkielet w `src/engine/` z testami, jeszcze **niepodłączony do reguł engine**;
- podgląd karty z ilustracją, autosave (`localStorage`) i wznawianie partii
  (z zapisu pola oraz z autosave).

Następny większy pakiet: wpięcie frameworków (activated/triggered abilities,
tworzenie tokenów przez efekty) do engine i katalogu syntetycznego, żeby działały
w realnej partii przez `legalCommands`. Realne karty czekają na listę właściciela
