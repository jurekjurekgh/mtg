...
- [ ] **P0: PR na starcie.** Ten plan jako osobny commit, push, otwarcie PR
  do `main` przed jakimkolwiek kodowaniem (ADR 0020 A).
  Kryterium: PR OPEN na GitHubie, opis z planem audytu.
- [ ] **P1: baza zmierzona.** `npm test` + `npm run build` na stanie `main`
...
- [ ] **P3: naprawy findings u root cause** (jeśli audyt coś znajdzie).
...
- [ ] **P4: pętla jakości** (ADR 0021 pkt 4, innymi ścieżkami niż sesja #106):
...
- [ ] **P5: domknięcie.** `npm run test:all`, `npm run build`, szybki
