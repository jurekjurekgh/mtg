...
5. **Attack restriction flying** — `cantAttackUnless { condition: 'defendingPlayerControlsFlying' }` na obiekcie; sprawdzane w `legalAttackerOptions`/`declareAttackers` (jak `cantAttackYou` z Vow). Lurking Green Dragon: warunek `defendingPlayer has creature with flying` na polu bitwy.
