...
export function createGameState({ seed, players }) {
  ...
  const state = {
    ...
    cardsDrawnThisTurn: {},
    // Ile życia każdy gracz ZYSKAŁ w bieżącej turze (Ulna Alley Shopkeep —
    // Infusion: „as long as you gained life this turn"). Zerowane przy
    // zmianie tury, jak cardsDrawnThisTurn.
    lifeGainedThisTurn: {},
    ...
    // wracającej z grobu, gdy legalnych gospodarzy jest więcej niż jeden.
    pendingAuraHost: null,
...
function outsideHandCastScope(card, {
  ...
  if (card.kind === 'spell') {
    ...
    if (!allowX && (spell.xCost || spell.fireball)) return false;
...
function firstPendingDecision(state) {
  ...
  if (state.pendingAuraHost) return { playerId: state.pendingAuraHost.playerId, kind: 'auraHost' };
...
export function execute(state, input) {
  ...
  if (state.pendingExileCast) {
    ...
    if (cmd.playerId !== state.pendingExileCast.playerId) return reject('exile_cast_not_your_decision');
    ...
    const castableKind = outsideHandCastScope(card, {
      allowTargets: true, allowModes: true, allowAdditionalCost: true, allowX: true, allowAura: true,
    });
    const decline = (note) => {
      ...
      state.events.push(event('exile_cast_resolved', {
        playerId: pending.playerId, objectId: pending.objectId, cardId: pending.cardId,
        declined: true, reason: note ?? null,
      }));
      return accepted(state, cmd, { ok: true, events: state.events.slice(before) });
    };
    ...
    try {
      ...
      state.events.push(event('exile_cast_resolved', {
        playerId: pending.playerId, objectId: pending.objectId, cardId: pending.cardId, declined: false,
      }));
      return accepted(state, cmd, { ok: true, events: state.events.slice(before) });
    } catch (error) {
      return reject(`illegal_exile_cast:${error.message}`);
    }
  }
  ...
  // wracająca z grobu wybiera zaczarowany obiekt PRZY WEJŚCIU. To nie celowanie
  // (hexproof/protection nie blokują), ale realna decyzja gracza, gdy legalnych
  // gospodarzy jest więcej niż jeden — przy jednym domyka się sama (effects.js).
  if (state.pendingAuraHost) {
    ...
    const before = state.events.length;
    state.pendingAuraHost = null;
    if (pending.restorePriorityTo && state.players.some((p) => p.id === pending.restorePriorityTo)) {
      state.turn.priorityPlayerId = pending.restorePriorityTo;
    }
    // Wejście wykonuje TA SAMA funkcja co ścieżka automatyczna (effects.js,
    // `returnPermanentFromGraveyardOutcome`) — jedno źródło reguły (L41).
...
export function playerView(state, playerId) {
    ...
    zones[zone] = ids.map((id) => {
      ...
      if (zone === 'stack') {
        return {
          ...
          // ani stół, ani bot nie wiedzą, CO dana kontra zdolności rozstrzyga — a to ta sama
  ...
  // M203/2: kolejność prezentacji = enumeracja (dawniej odwracał unshift).
  ...
  } else if (state.status === 'active' && !blockedByOthersDecision && activeAuraHost) {
    ...
    // stać. Zakres tożsamy z bramką resolve_exile_cast (L48: oferta = walidacja
    // — inaczej reject i crash bota). Land i karty spoza zakresu → sama
    ...
    const exileCastable = outsideHandCastScope(exileCard, {
      allowTargets: true, allowModes: true, allowAdditionalCost: true, allowX: true, allowAura: true,
    });
    ...
    if (exileCastable && exileCard.zone === 'exile') {
      // Jeden wzorzec komendy dla WSZYSTKICH wariantów rzutu (cele, tryb,
      // koszt dodatkowy, X, gospodarz aury) — L48: oferta niesie dokładnie to,
      // co potem rozlicza bramka `resolve_exile_cast`.
      const pushExileCast = (offer) => {
        legalCommands.push(command('resolve_exile_cast', playerId, {
      ...
      if (exileCard.aura || exileCard.bestow) {
        // Aura (CR 303.4a, znalezisko E audytu PR #93): gospodarza wybiera
        // gracz, a wariant bestow ma WŁASNY koszt — jedno i drugie wylicza
        ...
        if (cost <= budget && hasColorForCardId(state, playerId, exileCard.cardId, 0)) {
          if (exileCard.kind === 'spell') {
            // Vaan idzie przez `castSpell`: płaci PEŁNY koszt, więc potrafi
  ...
  if (state.status === 'active' && firstDecisionOwner == null && state.pendingMulligans.length === 0 && !state.pendingMulliganBottom && !state.pendingScry && !state.pendingSurveil
    ...
    if (state.turn.activePlayerId === playerId
      ...
      // w PÓŹNIEJSZEJ turze (CR 702.185a) i już nie za koszt warp — warp to
      // koszt alternatywny wyłącznie z ręki (znalezisko J audytu PR #93),
      // więc z exile płaci się koszt many, jak za zaplotowaną kartę bez
      // zniżki. Oferta = `cast_permanent`, ta sama komenda co z ręki.
