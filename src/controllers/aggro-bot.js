...
export function createAggroBot() {
  ...
  return Object.freeze({
    chooseCommand(view) {
      ...
      // 2026-09-14f (zgłoszenie właściciela): mulligan londyński — aggro
      // trzymał dotąd ZAWSZE pierwszą ofertę (keep), także rękę 0-lądową.
      // Polityka jak w heuristic-bocie: keep ⇔ ≥2 lądy w ręce albo cap
      // 2 mulliganów; przy odkładaniu na spód trzymaj lądy, oddawaj
      // najdroższe czary (reguła jednej przyczyny mulligana: brak many).
      {
        const mulligans = byType(view, 'resolve_mulligan_choice');
        if (mulligans.length) {
          const lands = view.zones.hand.filter((o) => (o.kind ?? '') === 'land').length;
          const taken = mulligans[0].mulligans ?? 0;
          const keepNow = lands >= 2 || taken >= 2;
          return mulligans.find((cmd) => cmd.keep === keepNow) ?? mulligans[0];
        }
        const bottoms = byType(view, 'resolve_mulligan_bottom_choice');
        if (bottoms.length) {
          ...
          const keepLoss = (ids) => ids.reduce((sum, id) => {
            ...
            if (isLand) return sum + 100;
            const body = 2 * (card.power ?? 0) + (card.toughness ?? 0);
            return sum + 60 - (card.manaCost ?? 0) * 5 + Math.min(body, 10);
          }, 0);
          let best = bottoms[0];
          let bestLoss = Infinity;
          for (const variant of bottoms) {
            const loss = keepLoss(variant.cardIds ?? []);
            if (loss < bestLoss) { bestLoss = loss; best = variant; }
          }
          return best;
        }
      }
      ...
      for (const type of simple) {
        ...
        if (type === 'resolve_exile_cast') {
          ...
          // wariant rzutu jest w ofercie; inaczej rezygnacja → Treasure.
          return byType(view, 'resolve_exile_cast').find((c) => c.cast) ?? found;
        }
      ...
      // Fallback dla dowolnej pozostałej decyzji blokującej (np. resolve_color_choice, resolve_index_choice)
      // — aggro bierze pierwszy wariant, jak heuristic (deterministycznie).
      const anyResolve = view.legalCommands.find((c) => c.type.startsWith('resolve_'));
