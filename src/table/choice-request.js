...
/**
 * M72 (Batch 29): wizard GENERYCZNEGO rozdzielania obrażeń niecombat (Fireball,
 * CR 119.4). Steppery +/− przy każdym celu; suma nie może przekroczyć total.
 * NIE enumerujemy kombinacji — gracz przypisuje każdemu celowi ilość.
 */
export function renderDamageDistributionWizard(host, { view, session, pending, defaultCommand, onComplete, onCancel }) {
  clearChoiceElement(host);
  const src = pending.sourceCardId ? session.nameOf(pending.sourceCardId) : 'Źródło';
  choiceNode(host, 'div', 'choice-request-intro',
    `Rozdziel ${pending.total} obrażeń (${src}) między cele — każdemu tyle, ile chcesz:`);
  const list = choiceNode(host, 'div', 'damage-wizard-list');
  const amounts = pending.targetIds.map(() => 0);
  const amountEls = [];
  const total = () => amounts.reduce((a, b) => a + b, 0);
  const remaining = () => pending.total - total();

  const render = () => {
    for (let idx = 0; idx < amountEls.length; idx += 1) {
      if (amountEls[idx]) amountEls[idx].textContent = String(amounts[idx]);
    }
    if (remainingEl) remainingEl.textContent = `pozostało do rozdziału: ${remaining()}`;
  };
  pending.targetIds.forEach((tId, idx) => {
    const isPlayer = view.players.some((p) => p.id === tId);
    const name = isPlayer ? session.nameOfPlayer?.(tId) ?? (tId === view.playerId ? 'Ty' : 'Nieprzyjaciel') : session.nameOfObject(tId);
    const row = choiceNode(list, 'div', 'damage-wizard-row');
    choiceNode(row, 'span', 'damage-wizard-name', name);
    const minus = choiceNode(row, 'button', 'ghost-btn damage-wizard-minus', '−1');
    minus.type = 'button';
    const amountEl = choiceNode(row, 'span', 'damage-wizard-amount', '0');
    amountEls.push(amountEl);
    const plus = choiceNode(row, 'button', 'ghost-btn damage-wizard-plus', '+1');
    plus.type = 'button';
    minus.addEventListener('click', () => {
      if (amounts[idx] <= 0) return;
  ...
  const remainingEl = choiceNode(list, 'div', 'damage-wizard-remaining', `pozostało do rozdziału: ${pending.total}`);

  const actions = choiceNode(host, 'div', 'choice-request-options');
  const confirm = choiceNode(actions, 'button', 'action choice-request-option damage-wizard-confirm', 'Zatwierdź przydział');
  confirm.type = 'button';
  confirm.addEventListener('click', () => {
    const assignments = pending.targetIds.map((tId, idx) => ({ targetId: tId, amount: amounts[idx] }));
    onComplete?.({ type: 'resolve_damage_distribution', playerId: view.playerId, assignments });
  });
  if (defaultCommand) {
    const def = choiceNode(actions, 'button', 'action choice-request-option damage-wizard-default', 'Domyślnie (po równo)');
    def.type = 'button';
    def.addEventListener('click', () => onComplete?.(defaultCommand));
  }
  if (onCancel) {
