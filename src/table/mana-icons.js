...
export function manaSymbolsHtml(text) {
  ...
  // B (znalezisko właściciela 2026-09-12, Abstruse Interference): grupą jest
  // każdy CIĄGŁY PRZEBIEG symboli, a NIE cały napis. Poprzednio CAŁOŚĆ
  // (wraz z prozą) lądowała w jednym nowrap .ms-group — przy założeniu, że
  // „każdy wywołujący przekazuje JEDEN koszt". Założenie złamał E3 (wpisy
  // modala Rozgrywka to pełne ZDANIA z kosztem w środku) — zdanie stawało
  // się jedną niełamliwą jednostką i rozpychało modal z scrollem w poziomie.
  // Proza zostaje na zewnątrz (łamie się normalnie); czysty koszt („{2}{W}")
  // to jeden przebieg, więc dotychczasowy output dla kosztów jest IDENTYCZNY.
  let run = [];
  const flushRun = () => {
    if (run.length === 0) return;
    out.push(`<span class="ms-group">${run.join('')}</span>`);
    run = [];
  };
  while ((match = pattern.exec(rest)) !== null) {
    if (match.index > last) {
      // Proza między symbolami kończy przebieg (osobne koszty).
      flushRun();
      out.push(escapeHtml(rest.slice(last, match.index)));
    }
    run.push(symbolSpan(match[0]));
