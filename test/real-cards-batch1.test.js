...
test('materializacja przenosi morph i entersWithCounters do obiektu gry', () => {
  const registry = createCardRegistry();
  assert.deepEqual(gameObjectDataOf(registry.get('highland-game')), { kind: 'creature', power: 2, toughness: 1, manaCost: 2, abilities: registry.get('highland-game').abilities });
