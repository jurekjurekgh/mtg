...
test('log opisuje decyzję devour (Gorger Wurm) — wymaganie i poświęcenie', () => {
  ...
  const session = createSession({ seed: 16, registry, decks });
  playOut(session);
...
test('log opisuje decyzję endure (Kin-Tree Nurturer) — wybór i tryb', () => {
  ...
  // Seed 2 po transzy 2 batcha 33 (green +2, black +1) — przelosowane hunterem.
...
test('log opisuje cel delirium (Fear of Burning Alive) — obrażenia w stwora', () => {
  ...
  // Seed 38 po transzy 2 batcha 33 (green +2) — przelosowane hunterem.
  const session = createSession({ seed: 38, registry, decks });
  playOut(session);
  const texts = logEventTexts(session);
