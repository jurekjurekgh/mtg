...
test('B3: przeciwnik z removalami i otwartą maną — bot nie atakuje wartościowego stwora', () => {
  ...
  const bot = createHeuristicBot({ seed: 1, opponentDeck: REMOVAL_DECK });
...
test('B3: przeciwnik może zabić atakującego — bot wstrzymuje atak; bez many — atakuje', () => {
  ...
  const botRisky = createHeuristicBot({ seed: 1, opponentDeck: REMOVAL_DECK });
  ...
  const botNoMana = createHeuristicBot({ seed: 1, opponentDeck: REMOVAL_DECK });
...
test('B3: przeciwnik BEZ czarów — bot atakuje mimo karty w ręce i many', () => {
  ...
  const bot = createHeuristicBot({ seed: 1, opponentDeck: NO_SPELL_DECK });
  const cmd = bot.chooseCommand(playerView(state, 'p1'));
...
test('B3: blok — przeciwnik z pumpem i maną, bot nie wchodzi w zabójczy blok', () => {
  ...
  const botPump = createHeuristicBot({ seed: 1, opponentDeck: PUMP_DECK });
  ...
  const botPlain = createHeuristicBot({ seed: 1, opponentDeck: NO_SPELL_DECK });
...
test('B3: determinizm — ten sam seed i talia dają te same decyzje', () => {
  const state = makeAttackState(2, 2, 3, 1, 1);
  const a = createHeuristicBot({ seed: 9, opponentDeck: REMOVAL_DECK });
  const b = createHeuristicBot({ seed: 9, opponentDeck: REMOVAL_DECK });
...
test('B3: partia z modelowaniem (obaj gracze znają talie) kończy się i jest deterministyczna', () => {
  const registry = createCardRegistry();
  const deck = parseDeckText(fs.readFileSync('decks/real-batch6.txt', 'utf8'), registry);
  const run = (seed) => {
    const state = setupCardMatch({ seed, players: [{ id: 'p1' }, { id: 'p2' }], decks: new Map([['p1', deck.cardIds], ['p2', deck.cardIds]]), registry });
    const result = runSimulation({
      state,
      controllers: new Map([
        ['p1', createHeuristicBot({ seed: seed + 1, opponentDeck: deck.cardIds })],
        ['p2', createHeuristicBot({ seed: seed + 2, opponentDeck: deck.cardIds })],
      ]),
      maxCommands: 3000,
    });
    return result.state;
