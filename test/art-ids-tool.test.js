...
test('lokalny słownik zawiera wszystkie karty z ID setu, bez ucieczek i z dubletami setów', () => {
  ...
  // Batch 52 (lista właściciela): +9 pozycji 580–588 → 588.
  // Batch 53 (lista właściciela), transza 6: +1 pozycja 592 → 596.
  // Batch 53, domknięcie: +2 pozycje 589 i 593 → 598.
  ...
  assert.equal(data.length, 553, 'pełna lista kolekcji (553 unikalne pozycje; FUS usunięte 2026-09-14)');
...
test('lokalny słownik (tools/collection-art-ids.csv) pokrywa karty z artId', () => {
  ...
  assert.equal(withArt.length, 475, 'wszystkie realne karty mają artId (Batche 1–55)');
