...
test('lokalny słownik zawiera wszystkie karty z ID setu, bez ucieczek i z dubletami setów', () => {
  ...
  // Batch 53 (lista właściciela), transza 5: +1 pozycja 597 → 595.
  assert.equal(data.length, 595, 'pełna lista kolekcji (595 unikalnych pozycji)');
...
test('lokalny słownik (tools/collection-art-ids.csv) pokrywa karty z artId', () => {
  ...
  // Batch 53, transza 5: +1 karta (597) → 452.
  assert.equal(withArt.length, 452, 'wszystkie realne karty mają artId (Batche 1–53, transza 5)');
