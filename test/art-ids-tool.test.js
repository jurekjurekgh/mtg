...
test('lokalny słownik zawiera wszystkie karty z ID setu, bez ucieczek i z dubletami setów', () => {
  ...
  // Batch 53 (lista właściciela), transza 2: +1 pozycja 598 → 592.
  assert.equal(data.length, 592, 'pełna lista kolekcji (592 unikalnych pozycji)');
...
test('lokalny słownik (tools/collection-art-ids.csv) pokrywa karty z artId', () => {
  ...
  // Batch 53, transza 2: +1 karta (598) → 449.
  assert.equal(withArt.length, 449, 'wszystkie realne karty mają artId (Batche 1–53, transza 2)');
