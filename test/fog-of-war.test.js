...
test('biblioteka przeciwnika nie ujawnia cardId ani kolejności kart', () => {
  ...
  assert.deepEqual(view.zones.library, [{ id: 'library-1', hidden: true }, { id: 'library-2', hidden: true }]);
...
test('własna biblioteka również nie trafia do zwykłego PlayerView', () => {
  ...
  assert.deepEqual(playerView(state, 'p1').zones.library, [{ id: 'library-1', hidden: true }]);
});
