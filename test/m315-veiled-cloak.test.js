 ...
 *
 * A1. Cloakowane stwory mają z automatu ward {2} (CR 701.58a) — badge kafla
 ...
 *     karty; po obrocie traci ward {2} (CR 701.58b). Dotąd brak komendy w
...
test('M315/A2: uncover kosztuje koszt many KARTY, przywraca cechy i zdejmuje ward (CR 701.58b)', () => {
  const state = game('p1');
  const cloaked = cloakedFrom(state, 'goblin-piker');
  addMana(state, 'p1', 5, { colors: ['R'] });
  ...
  assert.equal(flipped.ward ?? null, null, 'po uncover cloak TRACI ward {2} (CR 701.58b)');
